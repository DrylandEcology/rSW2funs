...
calc_SMTRs <- function(
  ...
  if (has_soil_temperature) {
    ...
    if (
      ...
      SMTR[["MCS_depth"]] <- adjustLayer_byImp(
      ...
      SMTR[["Lanh_depth"]] <- adjustLayer_byImp(
        depths = c(10, 70),
        imp_depth = imp_depth,
        sdepths = soildat[, "depth_cm"]
      )
      ...
      if (SMTR[["SMR_normalyears_N"]] > 0) {
        ...
        if (calc50) {
          weights50 <- calc_weights_from_depths(
            i_depth50,
            target_cm = SMTR[["Fifty_depth"]],
            depths_cm = soildat[, "depth_cm"]
          )

          soildat <- t(add_layer_to_soil(
            x = t(soildat),
            il = i_depth50,
            w = weights50
          ))
          ...
          vwc_dy_nrsc[["val"]] <- add_layer_to_soil(
            vwc_dy_nrsc[["val"]],
            il = 2 + i_depth50,
            w = weights50
          )
        ...
        if (any(calcMCS)) {
          for (k in which(calcMCS)) {
            weightsMCS <- calc_weights_from_depths(
              i_MCS[k],
        ...
        if (any(calcLanh)) {
          for (k in which(calcLanh)) {
            weightsLanh <- calc_weights_from_depths(
              i_Lanh[k],
